                            ...
                            <div className="space-y-6 py-8 text-base leading-7 text-gray-600">
                                ...
                                <div className="space-y-4">
                                    {currentQuestion.answers.map((answer, index) => (
                                        <button
                                            key={index}
                                            className={`${selectedAnswer === answer.id ? 'bg-green-500' : 'bg-white'}
                                            bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110`}
                                            onClick={() => handleAnswerSelect(answer.id)}
                                        >
                                            <div className="flex items-center space-x-2">
                                                <svg
                                                    className="h-6 w-6 flex-none fill-sky-100 stroke-sky-500 stroke-2 mr-2"
                                                    strokeLinecap="round"
